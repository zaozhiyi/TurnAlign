...
- The built wheel passes all 354 tests from site-packages on Python 3.12 with
...
- GitHub CI additionally covers Python 3.11, 3.13 and 3.14 on Ubuntu, macOS
  and Windows with `websockets` 17.1; the Ubuntu 3.13 and 3.14 jobs also run
  the optimized (`python -O`) suite.
...
- The package job creates a validated CycloneDX 1.6 SBOM from an isolated
  ...
  rejects a missing or structurally incomplete SBOM.
...
- Gate reports and bounded lock/SBOM inputs are validated from the exact bytes
  ...
  read, preventing a check/hash time-of-use mismatch.
...
- Server policy allowlists require immutable sets of trimmed strings; trust
  switches require real booleans and session duration rejects booleans. Client
  backend, model, language, compute and component identities remain strings
  instead of being coerced into allowlisted names.
...
- Core audio and transcript models apply the same strict runtime typing to PCM,
  timestamps, protocol counters, confidence values and event kinds. JSON boolean
  values therefore cannot masquerade as numeric release or quality evidence.
- Event JSONL, WebSocket controls and deployment-probe responses, gate reports
  and SBOM inputs share strict JSON semantics: duplicate keys and
  `NaN`/`Infinity` are rejected, while generated evidence refuses non-standard
  numeric output and remains atomically written. Deployment probes also reject
  binary control frames.
- whisper.cpp result JSON is read through a 64 MiB cap, rejects duplicate keys
  and non-standard numbers, and requires typed transcript objects, text and
  finite non-negative timestamps before constructing core hypotheses.
...
- A dependency-free `py3-none-any` wheel and source distribution build
  ...
  and the temporary pip/setuptools bootstrap versions are fixed, then removed
  before the runtime SBOM is generated. First-party GitHub Actions are pinned to immutable
  commit hashes with checkout credential persistence disabled. CI downloads
  the pinned actionlint release only after verifying its published SHA-256,
  then validates every workflow file.
