...
turnalign host-profile --source-commit "$(git rev-parse HEAD)" \
...
profile must be generated on the target host and bind its platform
inventory, source commit, and exact name, size, and SHA-256 identity of every
other retained artifact. Every gate report must name the same source commit,
...
path-safe pure-Python TurnAlign archive whose console entry point and every
`RECORD` SHA-256 digest and size independently verify;
