...
profile must be generated by the active candidate command on the target host
and bind its installed Wheel version, embedded source identity, versioned Python
paths, platform inventory, source commit, and exact name, size, and SHA-256
identity of every other retained artifact. Every gate report must name the same source commit,
