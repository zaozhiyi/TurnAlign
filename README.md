...
`host-profile` 的第 4 版证据还会逐文件哈希目标机上实际运行的 `turnalign/` 包；聚合门禁要求其路径、文件集、大小和 SHA-256 与保留 Wheel 完全一致。源码目录遮蔽、缺失/替换文件、符号链接、可写安装文件和额外 `__pycache__`/`.pyc` 均会失败。以 root 运行演练时必须保留示例中的 `PYTHONDONTWRITEBYTECODE=1`。
...
missing files, symlinks, writable files and added bytecode. Root-run rehearsal
commands must keep `PYTHONDONTWRITEBYTECODE=1`. Every gate report must name the
same source commit,
