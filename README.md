...
turnalign host-profile --source-commit "$(git rev-parse HEAD)" \
  ...
  --artifact nginx-config=/etc/nginx/nginx.conf \
...
a non-root identity, graceful restart/shutdown, and least privilege;
